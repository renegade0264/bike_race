...
import { idlFactory } from "../candid/ledger.did";
...
function App() {
  ...
  const initLedgerActor = async (canisterId, identity) => {
    try {
      const args = {};
      ...
      console.error("initLedgerActor - error", e);
    }
  };
  ...
  const fetchBalance = async () => {
    ...
    if (walletType === "plug") {
      ...
      const actor = initLedgerActor(canisterId);
      ...
      const balance = await actor.icrc1_balance_of({
        owner: Principal.fromText(
          "44eio-obkrg-k6wts-mqhod-67syw-ksqdc-gprqi-zs5en-s3lk2-2ecmq-6ae"
        ),
        subaccount: [],
      });
      console.log(balance);
